            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                try:
                    ...
                    if df2.empty:
                        ...
                        if len(avg_delay_by_year) > 1:
                            slope, _ = np.polyfit(avg_delay_by_year['year'], avg_delay_by_year['delay'], 1)
                            answer2 = slope
                        else:
                    ...
                    keys_from_prompt = task_description.split('{\n')[1].split('\n}')[0].strip().split('\n')
                    keys = [k.split(':')[0].strip().replace('"', '') for k in keys_from_prompt]
