    ...
    try:
        ...
        if "highest-grossing" in task_lower:
            ...
            if not isinstance(df, pd.DataFrame):
                 return {"error": "Failed to scrape or process Wikipedia data.", "details": df}
        ...
        # --- RECIPE 2: Indian High Court Judgements (Live Query with Timeout) ---
        elif "indian high court" in task_lower:
            print("INFO: Running LIVE recipe for Indian Court data with timeout handler.")
            
            QUERY_TIMEOUT_SECONDS = 30 # Set a timeout shorter than the platform's limit
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                try:
                    ...
                    if df2.empty:
                        answer2, answer3 = "No data for court 33_10 in 2023.", "No data to plot."
                    else:
                        ...
                        df2.dropna(inplace=True, subset=['date_of_registration', 'decision_date'])
                        df2['delay'] = (df2['decision_date'] - df2['date_of_registration']).dt.days
                        df2 = df2[df2['delay'] >= 0]
                        
                        if len(df2.groupby('year')['delay'].mean()) > 1:
                            slope, _ = np.polyfit(df2.groupby('year')['delay'].mean().index, df2.groupby('year')['delay'].mean().values, 1)
                            answer2 = slope
                        else:
        ...
        return {"error": f"A critical error occurred: {e}, {traceback.format_exc()}"}